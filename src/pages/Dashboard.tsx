...
const Dashboard = () => {
  ...
  return <div className="min-h-screen bg-background pb-16 md:pb-0">
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <button onClick={() => setShowAddDialog(true)} className="hidden md:flex fixed bottom-6 right-6 w-14 h-14 bg-primary text-primary-foreground rounded-full shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 items-center justify-center z-40" aria-label="Add transaction">
          <Plus className="w-6 h-6" />
        </button>

        <AddTransactionDialog open={showAddDialog} onOpenChange={setShowAddDialog} onSuccess={handleTransactionAdded} />
      </main>
    </div>;
};
